...
(function (root) {
  ...
  function layoutChecklistTracker(slide, pres, content, C, theme) {
    ...
    items.slice(0, maxItems).forEach(function (item, i) {
      ...
      var rowFill = (i % 2 === 0) ? (dark ? C.card : 'F8FAFC') : (dark ? C.bg : (C.white || 'FFFFFF'));
      ...
      slide.addShape(pres.shapes.ROUNDED_RECTANGLE, {
        ...
        line: { color: item.checked ? accent : (dark ? '475569' : 'CBD5E1'), width: 1.5 },
        rectRadius: 0.04
      });
    ...
    slide.addShape(pres.shapes.ROUNDED_RECTANGLE, {
      ...
      fill: { color: dark ? '334155' : 'E2E8F0' }, rectRadius: barH / 2
    });
    // Fill
    if (progress > 0) {
      slide.addShape(pres.shapes.ROUNDED_RECTANGLE, {
  ...
  function layoutTwoColumnCompare(slide, pres, content, C, theme) {
    ...
    var colAColor = content.colA.color || '3B82F6';
    var colBColor = content.colB.color || 'EF4444';
    ...
    function renderCol(col, x, accent) {
      var tintBg = dark ? C.card : (accent === colAColor ? 'EFF6FF' : 'FEF2F2');
      ...
      items.forEach(function (item, i) {
        ...
        if (i < items.length - 1) {
          slide.addShape(pres.shapes.LINE, {
            ...
            line: { color: dark ? '334155' : 'E2E8F0', width: 0.5 }
          });
        }
      });
    }
  ...
  function layoutPhaseTimeline(slide, pres, content, C, theme) {
    ...
    var defaultColors = ['3B82F6', 'F59E0B', '0D9488', '8B5CF6'];
    ...
    if (hasVocab) {
      ...
      vocabText.forEach(function (v, i) {
        ...
        if (i < vocabText.length - 1) {
          vocabRuns.push({ text: ' · ', options: { fontSize: K.FONT_CAPTION, color: dark ? '475569' : 'CBD5E1' }});
        }
      });
  ...
  function layoutProblemWorkspace(slide, pres, content, C, theme) {
    ...
    slide.addShape(pres.shapes.RECTANGLE, {
      ...
      line: { color: forceDark ? '334155' : 'CBD5E1', width: 1, dashType: 'dash' }
    });
    ...
    for (var r = 0; r < dotRows; r++) {
      for (var c = 0; c < dotCols; c++) {
        slide.addShape(pres.shapes.OVAL, {
          ...
          w: 0.03, h: 0.03,
          fill: { color: forceDark ? '334155' : 'E2E8F0' }
        });
      }
    }
  ...
  function layoutRubricBands(slide, pres, content, C, theme) {
    ...
    var levelW = 0.9;
    var defaultBandColors = ['2563EB', '0D9488', 'F59E0B', 'EF4444'];
  ...
  function layoutDashboardData(slide, pres, content, C, theme) {
    ...
    if (hasStats) {
      ...
      var defaultStatColors = ['0D9488', '3B82F6', 'F59E0B', 'EF4444'];
