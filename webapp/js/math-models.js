...
const MathModels = (() => {
    ...
    function wrap(w, h, bg, body) {
        return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w} ${h}" width="${w}" height="${h}" style="font-family:Arial,Helvetica,sans-serif;"><rect width="${w}" height="${h}" fill="${bg||C.white}" rx="6"/>${body}</svg>`;
    }
    function t(x, y, str, o={}) {
        const sz  = o.sz    || 13;
        const col = o.col   || C.dark;
        const wt  = o.bold  ? 'bold' : 'normal';
        const anc = o.anc   || 'middle';
        const dy  = o.dy    || 0;
        const it  = o.italic? 'italic' : 'normal';
        str = String(str).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
        return `<text x="${x}" y="${y}" dy="${dy}" text-anchor="${anc}" font-size="${sz}" font-weight="${wt}" font-style="${it}" fill="${col}">${str}</text>`;
    }
    function r(x, y, w, h, o={}) {
        const fill   = o.fill   || 'none';
        const stroke = o.stroke || C.navy;
        const sw     = o.sw     !== undefined ? o.sw : 2;
        const rx     = o.rx     || 0;
        const op     = o.op     !== undefined ? o.op : 1;
        return `<rect x="${x}" y="${y}" width="${w}" height="${h}" fill="${fill}" fill-opacity="${op}" stroke="${stroke}" stroke-width="${sw}" rx="${rx}"/>`;
    }
    ...
    function circ(cx,cy,rad,o={}) {
        ...
        const sw     = o.sw     !== undefined ? o.sw : 2;
        const op     = o.op     !== undefined ? o.op : 1;
        return `<circle cx="${cx}" cy="${cy}" r="${rad}" fill="${fill}" fill-opacity="${op}" stroke="${stroke}" stroke-width="${sw}"/>`;
    }
    function arrow(x1,y1,x2,y2,col,sw=2) {
        const id = 'a'+Math.floor(Math.random()*99999);
        return `<defs><marker id="${id}" markerWidth="10" markerHeight="7" refX="9" refY="3.5" orient="auto"><polygon points="0 0,10 3.5,0 7" fill="${col}"/></marker></defs><line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${col}" stroke-width="${sw}" marker-end="url(#${id})"/>`;
    }
    function poly(pts, fill, stroke, op=1, sw=2) {
        return `<polygon points="${pts.map(p=>p[0]+','+p[1]).join(' ')}" fill="${fill}" fill-opacity="${op}" stroke="${stroke}" stroke-width="${sw}"/>`;
    }
    ...
    function numberLine(o={}) {
        ...
        const pts = o.points || [];
        const W=600, H=180, lx=60, rx=540, y=88;
        const px = v => lx + ((v-min)/(max-min))*(rx-lx);
        const parts = [];
        if (hlS!==null && hlE!==null)
            parts.push(`<rect x="${px(hlS)}" y="${y-15}" width="${px(hlE)-px(hlS)}" height="30" fill="${C.amber}" fill-opacity=".3" rx="4"/>`);
        parts.push(arrow(lx-12,y,rx+14,y,C.navy,2.5));
        for (let v=min; v<=max; v+=step) {
            const x=px(v), maj=Number.isInteger(v);
            parts.push(ln(x,y-(maj?13:7),x,y+(maj?13:7),{col:C.navy,sw:maj?2:1}));
            if (maj) parts.push(t(x,y+30,v,{sz:13,col:C.dark}));
        }
        for (const p of pts) {
            const x=px(p.val), c=p.color||C.pink;
            parts.push(circ(x,y,7,{fill:c,stroke:C.white,sw:2}));
            if (p.label) parts.push(t(x,y-22,p.label,{sz:12,col:c,bold:true}));
        }
    ...
    function fractionBar(o={}) {
        ...
        const W=580, H=190, bx=50, by=62, bw=480, bh=72;
        const pw=bw/d;
        const p=[];
        p.push(t(W/2,40,`${n}/${d}`,{sz:24,bold:true,col:C.navy}));
        for (let i=0;i<n;i++)
            p.push(r(bx+i*pw,by,pw,bh,{fill:col,op:.72,stroke:C.white,sw:2,rx:2}));
        p.push(r(bx,by,bw,bh,{fill:'none',stroke:C.navy,sw:3}));
        for (let i=1;i<d;i++)
            p.push(ln(bx+i*pw,by,bx+i*pw,by+bh,{col:C.navy,sw:2}));
        for (let i=0;i<d;i++)
            p.push(t(bx+i*pw+pw/2,by+bh+22,`1/${d}`,{sz:11,col:C.mid}));
        return wrap(W,H,C.white,p.join(''));
    }
    ...
    function fractionCircle(o={}) {
        const n=o.numerator??3, d=o.denominator??4;
        const col=o.color||C.amber;
        const W=340, H=340, cx=170, cy=175, R=145;
        ...
        for (let i=0;i<d;i++) {
            ...
            const fill=i<n?col:C.light, op=i<n?.72:1;
            p.push(`<path d="M${cx},${cy} L${x1},${y1} A${R},${R} 0 ${la},1 ${x2},${y2} Z" fill="${fill}" fill-opacity="${op}" stroke="${C.navy}" stroke-width="2.5"/>`);
        }
        p.push(t(cx,cy+9,`${n}/${d}`,{sz:30,bold:true,col:C.navy}));
        return wrap(W,H,C.white,p.join(''));
    }
    ...
    function ratioTable(o={}) {
        ...
        const W=420, H=310, tx=70, ty=50, tw=280, rh=42, cw=tw/2;
        const p=[];
        p.push(r(tx,ty,cw,rh,{fill:C.navy,stroke:C.navy,sw:0}));
        p.push(r(tx+cw,ty,cw,rh,{fill:C.navy,stroke:C.navy,sw:0}));
        p.push(t(tx+cw/2,ty+27,cA,{sz:17,bold:true,col:C.white}));
        p.push(t(tx+cw+cw/2,ty+27,cB,{sz:17,bold:true,col:C.white}));
        for (let i=1;i<=rows;i++) {
            const ry=ty+i*rh, bg=i%2===0?C.navyLight:C.white;
            p.push(r(tx,ry,cw,rh,{fill:bg,stroke:C.navy,sw:1}));
            p.push(r(tx+cw,ry,cw,rh,{fill:bg,stroke:C.navy,sw:1}));
        ...
        p.push(t(W/2,H-8,`Ratio ${rA}:${rB}  →  Unit Rate: ${ur}`,{sz:12,col:C.gray}));
        return wrap(W,H,C.white,p.join(''));
    }
    ...
    function coordinatePlane(o={}) {
        ...
        // Axes
        p.push(arrow(gx(min)-8,cy,gx(max)+20,cy,C.navy,2.5));
        p.push(arrow(cx,gy(min)+8,cx,gy(max)-20,C.navy,2.5));
        p.push(t(gx(max)+24,cy+5,'x',{sz:16,bold:true,col:C.navy}));
        p.push(t(cx+5,gy(max)-24,'y',{sz:16,bold:true,col:C.navy}));
        // Ticks
        for (let v=min;v<=max;v++) {
            if (!v) continue;
            p.push(ln(gx(v),cy-5,gx(v),cy+5,{col:C.navy,sw:1.5}));
            p.push(ln(cx-5,gy(v),cx+5,gy(v),{col:C.navy,sw:1.5}));
            if (v%2===0) {
                p.push(t(gx(v),cy+20,v,{sz:11,col:C.dark}));
                p.push(t(cx-14,gy(v)+4,v,{sz:11,col:C.dark,anc:'end'}));
            }
        }
        // Optional line
        if (lineEq) {
        ...
        // Points
        for (const pt of pts) {
            p.push(circ(gx(pt.x),gy(pt.y),6,{fill:pt.color||C.pink,stroke:C.white,sw:2}));
            if (pt.label) p.push(t(gx(pt.x)+12,gy(pt.y)-10,pt.label,{sz:12,bold:true,col:pt.color||C.pink}));
        }
        return wrap(W,H,C.white,p.join(''));
    }
    ...
    function pythagorean(o={}) {
        ...
        const p=[];
        // a² square (left)
        p.push(r(tx-a*sc,ty-a*sc,a*sc,a*sc,{fill:C.blue,op:.28,stroke:C.blue,sw:2}));
        p.push(t(tx-a*sc/2,ty-a*sc/2+6,`a²=${a*a}`,{sz:13,bold:true,col:C.blue}));
        // b² square (bottom)
        p.push(r(tx,ty,b*sc,b*sc,{fill:C.teal,op:.28,stroke:C.teal,sw:2}));
        p.push(t(tx+b*sc/2,ty+b*sc/2+6,`b²=${b*b}`,{sz:13,bold:true,col:C.teal}));
        // c² (rotated — use transform)
        const cLen=c*sc, ang=Math.atan2(a,b)*180/Math.PI;
        p.push(`<rect x="${p2[0]}" y="${p2[1]-cLen}" width="${cLen}" height="${cLen}" fill="${C.amber}" fill-opacity=".28" stroke="${C.amber}" stroke-width="2" transform="rotate(${-ang} ${p2[0]} ${p2[1]})"/>`);
        const midCx=(p2[0]+p3[0])/2+26, midCy=(p2[1]+p3[1])/2;
        p.push(t(midCx,midCy,`c²=${Math.round(c*c)}`,{sz:13,bold:true,col:C.amber}));
        // Triangle
        p.push(poly([p1,p2,p3],C.navy,'#1E2761',.12,3));
        // Right angle marker
        const rs=15;
        p.push(`<path d="M${p1[0]+rs},${p1[1]} L${p1[0]+rs},${p1[1]-rs} L${p1[0]},${p1[1]-rs}" fill="none" stroke="${C.navy}" stroke-width="2"/>`);
        // Side labels
        p.push(t(p1[0]-18,(p1[1]+p3[1])/2,`a=${a}`,{sz:14,bold:true,col:C.blue,anc:'end'}));
        p.push(t((p1[0]+p2[0])/2,p1[1]+22,`b=${b}`,{sz:14,bold:true,col:C.teal}));
        p.push(t(midCx-60,midCy+30,`c=${c}`,{sz:14,bold:true,col:C.amber}));
        p.push(t(W/2,H-12,`a² + b² = c²   →   ${a*a} + ${b*b} = ${Math.round(c*c)}`,{sz:15,bold:true,col:C.navy}));
        return wrap(W,H,C.white,p.join(''));
    }
    ...
    function boxPlot(o={}) {
        ...
        const W=580, H=210, lx=60, rx=520, y=88, bh=52;
        const px=v=>lx+((v-minV)/(maxV-minV||1))*(rx-lx);
        const p=[];
        p.push(t(W/2,26,'Box Plot',{sz:15,bold:true,col:C.navy}));
        p.push(ln(lx-10,y+bh/2,rx+10,y+bh/2,{col:C.gray,sw:1.5}));
        // Box
        p.push(r(px(q1),y,px(q3)-px(q1),bh,{fill:C.tealLight,stroke:C.teal,sw:2.5}));
        // Median
        p.push(ln(px(med),y,px(med),y+bh,{col:C.navy,sw:3.5}));
        // Whiskers
        p.push(ln(px(minV),y+bh/2,px(q1),y+bh/2,{col:C.teal,sw:2}));
        p.push(ln(px(q3),y+bh/2,px(maxV),y+bh/2,{col:C.teal,sw:2}));
        p.push(ln(px(minV),y+10,px(minV),y+bh-10,{col:C.teal,sw:2}));
        p.push(ln(px(maxV),y+10,px(maxV),y+bh-10,{col:C.teal,sw:2}));
        // Labels
        for (const [v,lbl] of [[minV,'Min'],[q1,'Q1'],[med,'Med'],[q3,'Q3'],[maxV,'Max']]) {
            p.push(t(px(v),y+bh+22,lbl,{sz:11,col:C.teal,bold:v===med||v===q1||v===q3}));
            p.push(t(px(v),y+bh+36,String(v),{sz:12,bold:true,col:C.navy}));
        }
        p.push(t(W/2,H-8,`IQR = ${q3-q1}   Range = ${maxV-minV}`,{sz:13,col:C.gray}));
        return wrap(W,H,C.white,p.join(''));
    }
